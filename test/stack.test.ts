...
describe('stack', () => {
  ...
  it(
    'should always be disabled if there is both x and x2',
      ...
      for (const stacked of [undefined, 'center', 'zero', 'normalize'] as StackOffset[]) {
        ...
        marks.forEach(mark => {
          ...
          assert.equal(
            ...
            log.message.cannotStackRangedMark(X),
  ...
  it(
    'should always be disabled if there is both y and y2',
    log.wrap(localLogger => {
      for (const stacked of [undefined, 'center', 'zero', 'normalize'] as StackOffset[]) {
        const marks = stacked === undefined ? STACK_BY_DEFAULT_MARKS : STACKABLE_MARKS;
        marks.forEach(mark => {
          ...
          assert.equal(
            warns[warns.length - 1],
            log.message.cannotStackRangedMark(Y),
            stringify({stacked: stacked, mark: mark})
          );
        });
      }
